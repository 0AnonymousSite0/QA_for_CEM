...
tokenizer = AutoTokenizer.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH", device_map="auto",
...
for year in years:
    ...
    for i in range(len(Questions)):
        ...
        print("No Question", i + 1, "\n", "Right_Answer:", Answers[i], "\nAnswer_from_CEM_knowledge-incorporated_fine-tuned_Qwen_7B_Chat:",
              answer1.strip().replace("\n", ""))
        df = pd.concat([df, df2], axis=0)
    save_df_to_excel(df, "All_answers_from_CEM_knowledge-incorporated_fine-tuned_Qwen_7B_Chat1" + year + ".xlsx", "sheet1")