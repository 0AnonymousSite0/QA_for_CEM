...
tokenizer = AutoTokenizer.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH",use_fast=False, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH", device_map="auto",torch_dtype=torch.bfloat16, trust_remote_code=True,top_p=0,
  temperature=0.5)
model.generation_config = GenerationConfig.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH")

#Replace Baichuan2-13B-Chat model
#tokenizer = AutoTokenizer.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH",revision="v2.0",use_fast=False, trust_remote_code=True)
#model = AutoModelForCausalLM.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH", revision="v2.0",device_map="auto",torch_dtype=torch.bfloat16, trust_remote_code=True,top_p=0,
#  temperature=0.5)
#model.generation_config = GenerationConfig.from_pretrained("YOUR_FINE_TUNED_MODEL_PATH",revision="v2.0")
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_CEM_knowledge-incorporated_fine-tuned_Baichuan2-7B-Chat:",answer1.strip().replace("\n",""))
    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "All_answers_from_CEM_knowledge-incorporated_fine-tuned_Baichuan2-7B-Chat"+year+".xlsx", "sheet1")