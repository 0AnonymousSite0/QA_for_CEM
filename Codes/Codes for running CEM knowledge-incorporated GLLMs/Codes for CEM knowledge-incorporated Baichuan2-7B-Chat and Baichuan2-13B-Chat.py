...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_CEM_knowledge-incorporated_Baichuan2-7B-Chat:",answer1.strip().replace("\n",""))
    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "All_answers_from_CEM_knowledge-incorporated_Baichuan2-7B-Chat"+year+".xlsx", "sheet1")