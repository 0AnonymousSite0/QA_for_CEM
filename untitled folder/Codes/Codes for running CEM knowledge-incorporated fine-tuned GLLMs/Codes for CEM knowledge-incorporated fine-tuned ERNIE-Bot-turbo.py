...
llm_fine_tuned_ERNIE_Bot_turbo = QianfanChatEndpoint(streaming=True, model="YOUR_FINE_TUNED_MODEL_ID",temperature=0.5,top_p=0,penalty_score=1)

retriever = vectorstore.as_retriever(search_type="similarity_score_threshold", search_kwargs={'score_threshold': 0.1})

from langchain.chains import RetrievalQA

qa_chain_llm_fine_tuned_ERNIE_Bot_turbo = RetrievalQA.from_chain_type(llm_fine_tuned_ERNIE_Bot_turbo, retriever=retriever, chain_type_kwargs={"prompt": QA_CHAIN_PROMPT}, return_source_documents=True)
...
for year in years:
  ...
  for i in range(len(Questions)):
    result_llm_fine_tuned_ERNIE_Bot_turbo = qa_chain_llm_fine_tuned_ERNIE_Bot_turbo({"query": Questions[i]})
    df2 = pd.DataFrame([{"Question":Questions[i],"Correct_Answer":Answers[i],
                         "Answer1":result_llm_fine_tuned_ERNIE_Bot_turbo['result'],"Score1":final_score(Questions[i],result_llm_fine_tuned_ERNIE_Bot_turbo['result'],Answers[i])}])


    print("No of Question",i+1,"\n","Right_Answer:",Answers[i],
          "\n"+"Answer_from_CEM_knowledge-incorporated_fine-tuned_ERNIE_Bot_turbo:",str(result_llm_fine_tuned_ERNIE_Bot_turbo['result']).strip().replace("\n",""))
    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "Answer_from_CEM_knowledge-incorporated_fine-tuned_ERNIE_Bot-turbo"+year+".xlsx", "sheet1")