...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    df2 = pd.DataFrame([{"Question":Questions[i],"Correct_Answer":Answers[i],
                         ...
                         "Answer2":answer2,"Score2":final_score(Questions[i],answer2,Answers[i]),}])
    print("No of Question",i+1,"\n","Right_Answer:",Answers[i],
          "\n"+"Answer_from_original_GPT-3.5-turbo:",str(answer1).strip().replace("\n",""),
          "\n"+"Answer_from_original_GPT-4:",str(answer2).strip().replace("\n",""))
    df = pd.concat([df, df2], axis=0)
  save_df_to_excel(df, "Answer_from_original_GPT-3.5-turbo_GPT-4"+year+".xlsx", "sheet1")